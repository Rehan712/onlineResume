	...
	(req, res) => {
		Profile.findOne({ user: req.user.id }, (err, profile) => {
			if (err) {
				res
					.status(404)
					.json({ message: 'your profile is not registered' });
			} else {
				Post.findById(req.params.id, (err, post) => {
					if (err) {
						res
						...
						if (!post) {
							res
								.status(400)
								.json({ message: 'post not available' });
						} else {
		...
		Profile.findOne({ user: req.user.id }, (err, profile) => {
			if (err) {
				...
				Post.findById(req.params.id, (err, post) => {
					if (
						post.likes.filter(
							item => req.user.id === item.user.toString()
						).length
					) {
						res
							.status(400)
			...
			if (err) {
				...
				Post.findById(req.params.id, (err, post) => {
					if (
						...
						res
							.status(400)
							.json({ message: 'you have not like this post' });
					} else {
			...
			if (err) {
				...
				Post.findById(req.params.id, (err, post) => {
					...
					if (err) {
						res
							.status(400)
							.json({ message: 'An Error Occur this post' });
					} else {
				...
				Post.findById(req.params.id, (err, post) => {
					if (err) {
						...
						removeIndex = post.comments
							.map(
								item =>
									item._id.toString() === req.params.commentId
							)
							.indexOf();
